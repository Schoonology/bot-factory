...
function Factory(options) {
  this.module = this.module || options.module || false
...
function _tryCompile(script) {
  ...
  try {
    this.module = require(script)
  } catch (e) {
    console.error('Failed to load module:', script)
    console.error('Error:', e.message || e)
    console.error('Falling back to running as an executable...')
    this.module = null
  }
}
  ...
  return function (err, data) {
    ...
    if (err) {
      if (typeof err === 'number') {
        console.error('Bad exit code:' + err)
        process.exit(err)
      }
